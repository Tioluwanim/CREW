        ...
        {clients.map((client) => (
          <Card key={client.id} className="p-5">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-base font-medium text-ink-900">{client.name}</h3>
              <span className="text-xs text-ink-500">{client.averagePaymentDays} day avg. payment</span>
            </div>
            <div className="grid grid-cols-3 gap-4 text-sm">
              <Field label="Projects" value={String(client.projectIds.length)} />
              <Field label="Total billed" value={formatNaira(client.totalBilled)} />
              <Field label="Total paid" value={formatNaira(client.totalPaid)} />
            </div>
          </Card>
        ))}
      </div>
